...
The backend is structured around a central orchestration loop that manages state and delegates tasks to specific tools.

```mermaid
graph TB
  subgraph "Backend Service"
    Orchestrator[Simple Agent Orchestrator]
    State[AgentState]
    LLM[GeminiClient]
    Tools[Search Tools]
    
    Orchestrator --> State : Reads/Writes
    Orchestrator --> LLM : Prompting
    Orchestrator --> Tools : Data Fetching
    Tools --> LLM : Context Injection
  end
```

## Architectural Patterns

- **Orchestration Loop**: The system uses a linear, state-machine-like orchestration flow (`Strategist` -> `Researcher` -> `Analyst` -> `Synthesizer`).
- **Server-Sent Events (SSE)**: Used effectively to provide "live" feedback to the user, bridging the gap between long-running LLM tasks and immediate UI responsiveness.
- **Stateless/Stateful Hybrid**: The backend handles per-request state (`AgentState`) entirely within the scope of the request handler, keeping the service itself stateless and horizontally scalable (assuming no persistence requirement).
